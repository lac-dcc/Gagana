...
# This function gets the binary size of given a file
...
# Given a folder containing binaries of different optimization levels, this function checks if there's a regression
...
# Compile all source codes of a given directory
def compile_each_c_file(directory):
    ...
    if not os.path.isdir(directory):
        print(f"Error: The directory '{directory}' doesn't exists.")
        return
    ...
    for c_file in c_files:
        ...
        for opt_flag in opt_flags:
            ...
            if result.returncode == 0:
                ...
                print(f"✓ {output_name} ({opt_flag}) compiled successfuly.")
            else:
                print(f"✗ Error compiling {c_file} with {opt_flag}.")
